...
consvar_t cv_sleep = {"cpusleep", "-2", CV_SAVE, sleeping_cons_t, NULL, -1, NULL, NULL, 0, 0, NULL};
	...
	if (server && (multiplayer || netgame))
	{
		...
		switch (gametype)
		{
			...
			case GT_TEAMMATCH:
				...
				{
					// default settings for match: timelimit 10 mins, no pointlimit but 7 mins is better 
			...
			case GT_CTF:
				if (!cv_timelimit.changed && !cv_pointlimit.changed) // user hasn't changed limits
				{
					// default settings for CTF: timelimit 7 and pointlimit 5
					CV_SetValue(&cv_timelimit, 7);
