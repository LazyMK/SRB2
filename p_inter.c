...
static void P_HitDeathMessages(player_t *player, mobj_t *inflictor, mobj_t *source)
{
	...
	if (source)
	{
			...
			else switch (inflictor->type)
			{
				case MT_PLAYER:
					if ((inflictor->player->powers[pw_shield] & SH_NOSTACK) == SH_BOMB)
						...
						str = M_GetText("%s%s's TAGGIN'-HAND %s %s.\n");
					break;
				case MT_SPINFIRE:
				...
				case MT_THROWNEXPLOSION:
					str = M_GetText("%s%s's EXPLOSTION ring %s %s.\n");
					break;
				...
				case MT_REDRING:
					if (inflictor->flags2 & MF2_RAILRING)
						...
						str = M_GetText("%s%s's THROWN-RED ring %s %s.\n");
					break;
				default:
		...
		else switch (source->type)
		{
			case MT_NULL:
				...
				case 42:
					deathonly = true;
					str = M_GetText("%s drowned. What a pleb\n");
					break;
				case 43:
					str = M_GetText("%s was %s by spikes. What a pleb\n");
					break;
				case 44:
					deathonly = true;
					str = M_GetText("%s was crushed. What a pleb\n");
					break;
				}
				break;
			case MT_EGGMANICO:
			case MT_EGGMANBOX:
				str = M_GetText("%s was %s by Eggman's nefarious TV magic. What a pleb\n");
				break;
			case MT_SPIKE:
				str = M_GetText("%s was %s by spikes. What a pleb\n");
				break;
			default:
				str = M_GetText("%s was %s by an environmental hazard. What a pleb\n");
				break;
		}
	}
	else
	{
		...
		if (P_PlayerTouchingSectorSpecial(player, 1, 2))
			str = M_GetText("%s was %s by chemical water. What a pleb\n");
		else if (P_PlayerTouchingSectorSpecial(player, 1, 3))
			str = M_GetText("%s was %s by molten lava. What a pleb\n");
		else if (P_PlayerTouchingSectorSpecial(player, 1, 4))
			str = M_GetText("%s was %s by electricity. What a pleb\n");
		else if (deadtarget)
		{
			deathonly = true;
			if (P_PlayerTouchingSectorSpecial(player, 1, 6)
			 || P_PlayerTouchingSectorSpecial(player, 1, 7))
				str = M_GetText("%s fell into a bottomless pit. What a pleb\n");
			else if (P_PlayerTouchingSectorSpecial(player, 1, 12))
				str = M_GetText("%s asphyxiated in space. What a pleb\n");
			else
				str = M_GetText("%s died. What a pleb\n");
		}
		if (!str)
			str = M_GetText("%s was %s by an environmental hazard. What a pleb\n");
	...
	if (G_TagGametype())
	{
		if (leveltime == (timelimitintics + 1))
		{
			for (i = 0; i < MAXPLAYERS; i++)
			{
				...
				CONS_Printf(M_GetText("Amazing!, %s received double points for surviving the round.\n"), player_names[i]);
...
void P_CheckSurvivors(void)
{
	...
	if (!taggers) //If there are no taggers, pick a survivor at random to be it.
	{
		...
		{
			CONS_Printf(M_GetText("The IT player has left the game. What a baby.\n"));
			if (server)
				SendNetXCmd(XD_EXITLEVEL, NULL, 0);
			...
			if (leveltime >= hidetime * TICRATE)
			{
				...
				else
				{
					if (!(target->player->pflags & PF_TAGGED))
					{
						...
						CONS_Printf(M_GetText("Ouch!. %s was found!\n"), player_names[target->player-players]);
						P_CheckSurvivors();
					}
				}
			}
...
void P_PlayerFlagBurst(player_t *player, boolean toss)
{
	...
	// Flag text
	{
		...
		if (toss)
			CONS_Printf(M_GetText("Huh? %s tossed the %c%s%c.\n"), plname, flagcolor, flagtext, 0x80);
		else
			CONS_Printf(M_GetText("Looks like %s dropped the %c%s%c.\n"), plname, flagcolor, flagtext, 0x80);
	}
