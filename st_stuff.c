...
static void ST_drawTagHUD(void)
{
	...
	if (leveltime < hidetime * TICRATE) //during the hide time, the seeker and hiders have different messages on their HUD.
	{
		...
		if (stplyr->pflags & PF_TAGIT && !stplyr->spectator)
			sprintf(pstext, "%s", M_GetText("WAITING FOR ALL THESE LOSERS TO HIDE..."));
		else
		{
			if (!stplyr->spectator) //spectators get a generic HUD message rather than a gametype specific one.
			{
				if (gametype == GT_HIDEANDSEEK) //hide and seek.
					sprintf(pstext, "%s", M_GetText("OSHIT, HIDE BEFORE TIME RUNS OUT!"));
				else //default
					sprintf(pstext, "%s", M_GetText("HIDE!!!"));
			}
			else
	...
	else
	{
		...
		if (stplyr->pflags & PF_TAGIT)
			sprintf(pstext, "%s", M_GetText("Welp, You are it"));
		else
		{
...
static void ST_overlayDrawer(void)
{
	...
	if (!hu_showscores) // hide the following if TAB is held
	{
		...
		if (G_RingSlingerGametype() && cv_overtime.value
			&& (leveltime > (timelimitintics + TICRATE/1)) && cv_timelimit.value && (leveltime/TICRATE % 2 == 0))
		{
			if (splitscreen)
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(168), 0, M_GetText("OH SHIT, OVERTIME!"));
			else
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(184), 0, M_GetText("OH SHIT, OVERTIME!"));
		}
	...
	if (!hu_showscores && !splitscreen && netgame && displayplayer == consoleplayer)
	{
		...
		else if (!G_PlatformGametype() && stplyr->playerstate == PST_DEAD && stplyr->lives) //Death overrides spectator text.
		{
			...
			if (respawntime > 0 && !stplyr->spectator)
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(132), V_HUDTRANSHALF, va(M_GetText("You'll respawn in: %d second%s."), respawntime, respawntime == 1 ? "" : "s"));
			else
		...
		{
			V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(60), V_HUDTRANSHALF, M_GetText("You're a spectator."));
			if (G_GametypeHasTeams())
				...
				V_DrawCenteredString(BASEVIDWIDTH/2, STRINGY(132), V_HUDTRANSHALF, M_GetText("You can't join the game until the stage has ended."));
			else
