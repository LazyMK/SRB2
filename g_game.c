...
// This little line of code is for the Compact Scorboard, From Lat's Chatexe Code
...
boolean G_Responder(event_t *ev)
{
		...
		&& (ev->data1 == KEY_F12 || ev->data1 == gamecontrol[gc_viewpoint][0] || ev->data1 == gamecontrol[gc_viewpoint][1]))
	{
		...
		else
		{
			...
			// tell who's the view
			CONS_Printf(M_GetText("Viewing: %s\n"), player_names[displayplayer]);
...
void G_ExitLevel(void)
{
	if (gamestate == GS_LEVEL)
	{
		...
		if (gametype != GT_COOP)
			CONS_Printf(M_GetText("Thank fuck, The round has ended.\n"));
