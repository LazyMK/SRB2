...
static menuitem_t MPauseMenu[] =
{
	...
	{IT_STRING  | IT_CALL,    NULL, "Change Map..."    , M_MapChange,           24},
	...
	{IT_CALL | IT_STRING,    NULL, "Return to Main Title", M_EndGame,            80},
...
static menuitem_t SR_PandorasBox[] =
{
	...
	{IT_STRING | IT_CALL, NULL, "Quit Game 2.0",     M_UltimateCheat,    130},
};
...
static menuitem_t MP_ServerMenu[] =
{
	...
	{IT_WHITESTRING|IT_CALL,         NULL, "Let's Go!",                 M_StartServer,     130},
};
...
{
	{IT_STRING | IT_CALL, NULL, "<Alone Mode>", M_ChooseRoom,   9},
...
static menuitem_t OP_Mouse2OptionsMenu[] =
{
	...
	{IT_STRING | IT_CVAR, NULL, "2nd Mouse Serial Port",
	                                                &cv_mouse2port,       20},
...
static menuitem_t OP_VideoOptionsMenu[] =
{
	...
	{IT_STRING | IT_CVAR,    NULL, "Quincunx Aliasing",   &cv_quincunx,   140},
...
static menuitem_t OP_GameOptionsMenu[] =
{
	...
	// Compact Rankings toggle from lat's chatexe code
	{IT_STRING | IT_CVAR, NULL, "Compact Rankings",		  &cv_compactscoreboard,130},
...
void M_Drawer(void)
{
	...
	{
		M_DrawTextBox((BASEVIDWIDTH/2) - (100), (BASEVIDHEIGHT/2) - (16), 23, 2);
		if (gamestate == GS_LEVEL && (P_AutoPause() || paused))
			V_DrawCenteredString(BASEVIDWIDTH/2, (BASEVIDHEIGHT/2) - (4), V_YELLOWMAP, "===Paused===");
		else
			V_DrawCenteredString(BASEVIDWIDTH/2, (BASEVIDHEIGHT/2) - (4), V_ORANGEMAP, "Uh.. Where's the mouse?");
	}
}
...
static void M_DrawRoomMenu(void)
{
	...
	if (itemOn == 0)
		rmotd = M_GetText("Don't connect to the Master Server and play alone...");
	else
...
static void M_DrawConnectMenu(void)
{
	...
	if (ms_RoomId < 0)
		V_DrawRightAlignedString(BASEVIDWIDTH - currentMenu->x, currentMenu->y + MP_ConnectMenu[mp_connect_room].alphaKey,
		                         V_YELLOWMAP, (itemOn == mp_connect_room) ? "<Select to change>" : "<Alone Mode>");
	else
		V_DrawRightAlignedString(BASEVIDWIDTH - currentMenu->x, currentMenu->y + MP_ConnectMenu[mp_connect_room].alphaKey,
	...
	if (currentMenu == &MP_ServerDef)
	{
		if (ms_RoomId < 0)
			V_DrawRightAlignedString(BASEVIDWIDTH - currentMenu->x, currentMenu->y + MP_ServerMenu[mp_server_room].alphaKey,
			                         V_YELLOWMAP, (itemOn == mp_server_room) ? "<Select to change>" : "<Alone Mode>");
		else
			V_DrawRightAlignedString(BASEVIDWIDTH - currentMenu->x, currentMenu->y + MP_ServerMenu[mp_server_room].alphaKey,
			                         V_YELLOWMAP, room_list[menuRoomIndex].name);
	}
...
static void M_ConnectIP(INT32 choice)
{
	...
	V_DrawCenteredString(BASEVIDWIDTH/2, BASEVIDHEIGHT/2, 0, "Hol' up, Connecting to server...");
	I_OsPolling();
	I_UpdateNoBlit();
	if (rendermode == render_soft)
		I_FinishUpdate(); // page flip or blit buffer
